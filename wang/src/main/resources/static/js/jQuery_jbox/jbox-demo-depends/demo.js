...
function demo_1_1() {
    $.jBox.info('<strong>jQuery jBox</strong><br />版本：v2.0<br />日期：2011-7-24<br />官网：<a target="_blank" href="http://kudystudio.com/jbox">http://kudystudio.com/jbox</a>');
}
function demo_1_2() {
...
function demo_3_1() {
    //加了个其它参数closed
    $.jBox.prompt('您要删除的用户处于<br>登陆状态，无法删除！', '嘉诚','warning');
    
}
function demo_3_1_1() {
    //加了个其它参数closed
    $.jBox.prompt('因您长时间未做操作，<br>系统已失效，请重新登陆！', '嘉诚','warning');
}
function demo_3_2() {
    var html = '已提交至：' +
                '<div class="msg-div">' +
                '<p><span>崔洪海</span><span>臧友维</span><span>郭洪志</span><span>龙彦清</span><span>魏茂义</span></p>' +
                '<p><span>闫海江</span><span>于　淼</span><span>周传宇</span><span>林　延</span><span>乔海峰</span></p>' +
                '<p><span>徐丽莹</span><span>李　恒</span><span>关盛文</span><span>韩　清</span><span>崔洪海</span></p>' +
                '</div>' +
                '<div class="msg-div no-p">' +
                '<p><span>臧友维</span><span>郭洪志</span><span>龙彦清</span><span>魏茂义</span><span>闫海江</span></p>' +
                '<p><span>于　淼</span><span>周传宇</span><span>林　延</span><span>乔海峰</span><span>徐丽莹</span></p>' +
                '<p><span>李　恒</span><span>关盛文</span></p>' +
                '</div>' +
                '<div class="msg-div msg-b"><span>共26人</span><button type="button" onclick="$(this).parent().prev().slideToggle()">查看全部</button></div>';
    $.jBox.success(html, '嘉诚', {isG: true });
}
function demo_3_3() {
...
function demo_3_7() {
    var submit = function (v, h, f) {
        if (v == true)
            jBox.tip('保存成功。', 'success', { isG: true });
        else
            jBox.tip('保存失败。', 'error');
        return true;
    };
    // 可根据需求仿上例子定义按钮
    $.jBox.confirm("此记录已被修改，<br>是否保存？", "提示", submit,{ buttons: { '是': true, '否': false} });
}
