...
workflow.readyProcessType = function(typeId,typeName){
	var flag = true;
	for(var i = 0 ; i < processTypeIdList.length ; i++){
...
workflow.doSubmit = function(opt){
	var treeObj = $.fn.zTree.getZTreeObj("processTemplateTree");
 ...
 * 根据类型名车查询流程列表
 */
workflow.getFlowsByType = function(typeName,divId){
	$("#"+divId).html("");
 ...
 * 根据类型标识查询流程列表
 */
workflow.getFlowsByIden = function(typeIden,divId){
	$("#"+divId).html("");
 ...
 * 根据类型标识查询流程列表(是否)
 */
workflow.isHasFlowsByIden = function(typeIden,divId){
	$("#"+divId).html("");
	if(typeIden==null||typeIden.length==0){
		msgBox.tip({
			type:"fail",
