...
(function($) {
    ...
    var methods = {
        ...
        init: function(options, swfUploadOptions) {
            ...
            return this.each(function() {
                ...
                var settings = $.extend({
                    // Required Settings
                    id: $this.attr('id'), // The ID of the DOM object // DOM对象的ie
                    swf: 'uploadify.swf', // The path to the uploadify SWF file // swf路径
                    uploader: 'uploadify.php', // The path to the server-side upload script //上传请求路径

                    // Options
                    auto: true, // Automatically upload files when added to the queue //上传时是否自动添加到列队
                    buttonClass: '', // A class name to add to the browse button DOM object //上传按钮的样式
                    buttonCursor: 'hand', // The cursor to use with the browse button //上传按钮的手势
                    buttonImage: null, // (String or null) The path to an image to use for the Flash browse button if not using CSS to style the button
                    buttonText: 'SELECT FILES', // The text to use for the browse button //按钮显示的名称
                ...
                if (flashInstalled) {
                    ...
                    if (!settings.queueID) {
                        ...
                        if(settings.queueVesselId!=''){
                        	 $('#'+settings.queueVesselId).append($queue);
                        }else{
                        	 $wrapper.after($queue);
                        }
    ...
    var handlers = {
        ...
        onUploadError: function(file, errorCode, errorMsg) {
            ...
            var errorString = '错误';//'Error';
            switch (errorCode) {
                case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
                    errorString = 'HTTP 错误 (' + errorMsg + ')';//'HTTP Error (' + errorMsg + ')';
                    break;
                case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:
                    errorString = '找不到上传的url';//'Missing Upload URL';
                    break;
                case SWFUpload.UPLOAD_ERROR.IO_ERROR:
                    errorString = 'IO 异常';// 'IO Error';
                    break;
                case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
                    errorString = '安全错误';//'Security Error';
                    break;
                case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
                   // alert('The upload limit has been reached (' + errorMsg + ').');
                	 alert('上传限制已达到 (' + errorMsg + ').');
                    errorString = '超过上传限制';//'Exceeds Upload Limit';
                    break;
                case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
                    errorString = '失败';//'Failed';
                    break;
                case SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND:
                    break;
                case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
                    errorString ='验证错误';// 'Validation Error';
                    break;
                case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
                    errorString ='取消的';// 'Cancelled';
                    this.queueData.queueSize -= file.size;
            ...
            if (settings.checkExisting) {
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 1) {
                            var overwrite = confirm('一个文件的名字为"' + file.name + '" 已经存在于服务器。.\n你想替换现有的文件?');
                            if (!overwrite) {
                                this.cancelUpload(file.id);
                                $('#' + file.id).remove();
        ...
        onUploadSuccess: function(file, data, response) {
            ...
            // 调用用户追加的自定义方法.在成功之前执行的方法.
            // 后台上传入库如果出现异常并被捕捉.那uploadify是不会进行错误处理的
            // 故此无论成功与否都验证一下返回码 如果 返回码为500 则上传不成功
            if(dataObj.code && dataObj.code=='500'){
                settings.upload_error_handler.call(this, file, -200, 500);
            }else{
                  // Call the default event handler
