...
*/
      ...
      this._compileFormat();
      this.widget = $(getTemplate(this.timeIcon, this.pickDate, this.pickTime, options.pick12HourFormat, this.pickSeconds, options.collapse)).appendTo(isLoadShow?this.$element:$("#scrollable"));
...
//                      console.log(this.startDate);
    ...
    place: function(){
        ...
        if (this._isInFixed()) {
          position = 'fixed';
          offset.top = tempInput.offset().top + 35-116;
          offset.left = tempInput.offset().left-225;
        }
        //判断右边距
        if ($window.width() < offset.left + this.widget.outerWidth()+85) {
          offset.right = $window.width() - offset.left - this.width + ((position == 'fixed')?155:18);
          offset.left = 'auto';
//          this.widget.addClass('pull-right');
          (position == 'fixed')?offset.left = tempInput.offset().left-((tempInput.width() > 120)?this.height:105):'';
        } else {
          offset.right = 'auto';
//          this.widget.removeClass('pull-right');
        }
        //判断下边据
        if ($window.height() < tempInput.offset().top + this.widget.outerHeight()) {
          offset.top = offset.top - (this.widget.outerHeight()+this.height+8);
//          this.widget.addClass('pull-bottom');
        } else {
//          this.widget.removeClass('pull-bottom');
        }
        //如果左边距和下边据的类都有的话则把箭头向下
//        if(this.widget.hasClass('pull-right') && this.widget.hasClass('pull-bottom')){
//           this.widget.addClass('pull-bottom-right'); 
//        }else{
//           this.widget.removeClass('pull-bottom-right');
//        }
        this.widget.css({
          position: position,
          top: offset.top+116,
          left: offset.left+225,
          right: offset.right
        });
    ...
    click: function(e) {
      ...
      if (target.length === 1) {
    	 // alert(target[0].nodeName.toLowerCase());
...
//	    		if($(document.activeElement).parents(".bootstrap-datetimepicker-widget").length==0){
//	    			this.hide();
//	    		};
	    	}
    	//}
