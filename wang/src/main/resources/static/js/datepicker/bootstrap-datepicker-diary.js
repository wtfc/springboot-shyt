...
*/
      ...
      this._compileFormat();
      this.widget = $(getTemplate(this.timeIcon, this.pickDate, this.pickTime, options.pick12HourFormat, options.pickSeconds, options.collapse)).appendTo(isLoadShow?this.$element:$("#scrollable"));
...
//      alert(this.pickDate);
//        console.log(this)
       refObjAndOperate =  this.$element.data('ref-obj');
        if(refObjAndOperate){
          ...
          if(tempString.length==2){
            ...
            if(refObj){
              ...
              if(refObjValue){
                ...
                if(operate=="gt"){
                  this.setStartDate(new Date(refObjDate+3600*24*1000));
                }
              }
            }
          }
    ...
    place: function(){
      ...
      var bodyH = $("#scrollable").scrollTop();
      var tempInput = this.$element;//input组件  
      //console.log(tempInput);
      if(this.height > 24){
          offset.top = (bodyH != 0)?tempInput.offset().top - 81 + bodyH:tempInput.offset().top - 81;
          offset.left = offset.left - this.height * 6.66;
      }else{
          offset.top = (bodyH != 0)?tempInput.offset().top - 86 + bodyH:tempInput.offset().top - 86;
          offset.left = offset.left - this.height * 9.4;
      }

      var $window = $("#scrollable");
      ...
      var tempthis = this.widget
      if (this._isInFixed()) {
        position = 'fixed';
        offset.top = tempInput.offset().top + 35;
        offset.left = tempInput.offset().left;

        var mBody = this.$element.closest('.modal-body')
        mBody.scroll(function(){
            tempthis.css({
            position: position,
            top: offset.top - mBody.scrollTop(),
            left: offset.left,
            right: offset.right
          });
        })
      ...
      if ($window.width() < offset.left + this.widget.outerWidth()) {
        offset.right = $window.width() - offset.left - this.width + 15;
        offset.left = 'auto';
    ...
    fillDate: function() {
      ...
      this.widget.find('.datepicker-days th:eq(1)').text(year
    	   + '年   ' + dates[this.language].months[month]);
//         + ' ' + dates[this.language].months[month]);
      ...
      this.viewStartDate = prevMonth.format('yyyy-MM-dd');
      ...
      this.viewEndDate = nextMonth.format('yyyy-MM-dd');
      nextMonth = nextMonth.valueOf();
      var html = [];
      var row;
      var clsName;
      var elId =  this.$element.attr('id');
	  var oldVal =  $('#'+elId+'Input').val();
	  var dateStr = this.getStartDate()+','+this.getEndDate();
...
//        clsName += ' disabled';//初始化时全置为不可用
	    /*if(oldVal!=dateStr){
		  clsName += ' disabled';
	    }*/
		var d = new Date(prevMonth);
		//alert(d.format('yyyy-MM-dd'));
        row.append('<td class="day' + clsName + '" id="'+d.format('yyyy-MM-dd')+'">' + prevMonth.getUTCDate() + '</td>');
...
//        'th:eq(1)').text(year + '-' + (year + 9)).end().find('td');
      ...
      yearCont.html(html);
	   ...
	   //alert(this.loadShow);
      //alert(this.getStartDate()+'||'+this.getEndDate());
	  //this.$element.attr('data',this.getStartDate().format('yyyy-MM-dd')+'||'+this.getEndDate().format('yyyy-MM-dd'))
	 /* var elId =  this.$element.attr('id');
	  var oldVal =  $('#'+elId+'Input').val();
	  var dateStr = this.getStartDate()+','+this.getEndDate();*/
	  if(oldVal!=dateStr){
		$('#'+elId+'Input').val(dateStr).change();
	  }else{
		  currentObject.showWorklogStatus();
	  }
    },
          ...
          switch(target[0].nodeName.toLowerCase()) {
            ...
            case 'td':
              if (target.is('.day')) {
                ...
                if(this.loadShow){
                  //alert(this.$element.data('callback'));
                  //eval(viewDate);
                  //alert(typeof(eval(this.$element.data('callback'))));
                  //var e = $.Event('date.callback')
...
/*    test:function(e){
    ...
    _attachDatePickerEvents: function() {
      ...
      if(this.loadShow){this.widget.show()}
      if (this.isInput) {
        this.$element.on({
  ...
  $.fn.datepicker_worklog = function ( option, val ) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data('datetimepicker', (data = new DateTimePicker(
          this, $.extend({}, $.fn.datepicker_worklog.defaults,options))));
      }
      if (typeof option === 'string') data[option](val);
    });
  };

  $.fn.datepicker_worklog.defaults = {
    maskInput: true,
    pickDate: true,
    pickTime: false,
    ...
    collapse: true,
    loadShow: false,
    viewStartDate:-Infinity,
    viewEndDate: Infinity,
  };
  $.fn.datepicker_worklog.Constructor = DateTimePicker;
  var dpgId = 0;
  var dates = $.fn.datepicker_worklog.dates = {
    "zh-CN": {
