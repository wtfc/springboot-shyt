...
(function ($, undefined) {
    ...
    if(typeof(iframePlace) == "undefined"){
    	var iframePlace = {};
    }

    function setIframePlace(obj){
    	iframePlace = obj;
    }

    function getIframePlace(){
    	return iframePlace;
    }
   ...
   */
    ...
    AbstractSelect2 = clazz(Object, {
        ...
        init: function (opts) {
            ...
            this.containerName = "s2name_"+(opts.element.attr("name") || "autogen"+nextUid());//NAME
            this.containerEventName = this.containerId.replace(/([.])/g, '_').replace(/([;&,\-\.\+\*\~':"\!\^#$%@\[\]\(\)=>\|])/g, '\\$1');
            this.container.attr("id", this.containerId);
            this.container.attr("name", this.containerName);//NAME
            ...
            search.on("focus", function () { search.addClass("select2-focused"); }); //添加搜索样式
            search.on("blur", function () { search.removeClass("select2-focused");});//去除搜索样式
    ...
    SingleSelect2 = clazz(AbstractSelect2, {
        ...
        // single
        showIframe: function(){
        	var offset = getIframePlace();
        	var iframeBodyCover = document.createElement("iframe");
            iframeBodyCover.id = "__iframeBodyTree";
            iframeBodyCover.style.backgroundColor= "transparent";
            iframeBodyCover.style.cssText = "position:absolute;top:"+offset.top+"px;left:"+offset.left+"px;width:"+offset.width+"px;height:243px;filter:alpha(opacity=0);z-index:88;display:block";
        ...
        onSelect: function (data, options) {
            ...
            //zjw 选中去掉验证信息 start
    		var currentId = this.container[0].id;
    		$("#"+currentId).removeClass("error"); 
    		$("#"+currentId).parent().next(".help-block").html("");
    		//zjw 选中去掉验证信息 end
        },
        // single
    ...
    MultiSelect2 = clazz(AbstractSelect2, {
        ...
        initContainer: function () {
            ...
            this.selection.on("click", ".select2-search-choice:not(.select2-locked)", function (e) {
                //killEvent(e);
            	//zjw 只读时IE8报JS错误
            	if(!_this.opts.element.prop("readonly")){
            		_this.search[0].focus();
                    _this.selectChoice($(this));
            	}
            });
            // rewrite labels from original element to focusser
            this.search.attr("id", "s2id_input_"+(_this.opts.element.attr("id") || "autogen"+nextUid()));//邮件2.3需求
            //this.search.attr("id", "s2id_autogen"+nextUid());//zjw
        ...
        updateSelection: function (data) {
            ...
            //self.addSelectedChoice2(data);
            self.postprocessResults();
        },
        //zjw -- 改写主要拼大的
        /*addSelectedChoice2: function (d) {
            var enableChoice = !d.locked, self = this;
            var items = "";
            $(d).each(function () {
            	var id = self.id(d), val = self.getVal();
            	var item = enableChoice ? "<li class='select2-search-choice'>":"<li class='select2-search-choice select2-locked'>";
            	item += "<div>"+this.text+"</div>" + (enableChoice ? "<a href='#' class='select2-search-choice-close' tabindex='-1'></a>":"");
                item += "</li>";
                items += item;
                val.push(id);
                self.setVal(val);
            });
            $(items).insertBefore(self.searchContainer);
        },*/
        // multi
        tokenize: function() {
        ...
        onSelect: function (data, options) {
    		...
    		var currentId = this.container[0].id;
    		$("#"+currentId).removeClass("error"); 
    		$("#"+currentId).parent().parent().next(".help-block").html("");
    		//zjw 选中去掉验证信息 end
    ...
    $.fn.select2.locales['zh-CN'] = {
         formatMatches: function (matches) { if (matches === 1) { return "一个结果是可用的,按回车键选中它."; } return matches + " 结果可用,使用向上和向下箭头键来浏览."; },
         ...
         formatInputTooLong: function (input, max) { var n = input.length - max; return "请删掉 " + n + " 个字符" + (n == 1 ? "" : "s"); },
