//初始化方法
var toaMachineRestartRoomScanModule = {};
//重复提交标识
toaMachineRestartRoomScanModule.subState = false;
//获取修改信息
toaMachineRestartRoomScanModule.get = function (id) {
	$.ajax({
		type : "GET",
		...
		success : function(data) {
			if (data) {
				//清除验证信息
				hideErrorMessage();
				toaMachineRestartRoomScanModule.clearForm();
				$("#toaMachineRestartRoomScanForm").fill(data);
...
toaMachineRestartRoomScanModule.spCall = function(data, controlId) {
	//id
	var id = "";
	//姓名
	var name = "";
...
//添加留言
toaMachineRestartRoomScanModule.restartExchange = function (hide) {
	if(toaMachineRestartRoomScanModule.subState)return;
		toaMachineRestartRoomScanModule.subState = true;
			...
			success : function(data) {
				...
				if(data.success == "true"){
					msgBox.tip({
						...
						callback:function(){
							loadrightmenu("/machine/toaMachineRestart/loadRoomScan.action?id="+ids+"", "", "/machine/toaMachineRestart/loadRoomScan.action?id="+ids+"");
						}
					});
...
//添加修改公用方法(机房)
toaMachineRestartRoomScanModule.saveOrModifyRoomScan = function (hide) {
	if(toaMachineRestartRoomScanModule.subState)return;
		toaMachineRestartRoomScanModule.subState = true;
	if ($("#toaMachineRestartRoomScanForm").valid()) {
		if ($("#id").val() != 0) {
			url = getRootPath()+"/machine/toaMachineRestart/updateRoomScan.action";
		}
		var formData = $("#toaMachineRestartRoomScanForm").serializeArray();
		jQuery.ajax({
			url : url,
			type : 'POST',
			cache: false,
...
//清空表单数据
toaMachineRestartRoomScanModule.clearForm = function(){
	$('#toaMachineRestartRoomScanForm')[0].reset();
	hideErrorMessage();
};