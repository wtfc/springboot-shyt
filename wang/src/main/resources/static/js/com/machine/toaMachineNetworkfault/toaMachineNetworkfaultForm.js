//初始化方法
var toaMachineNetworkfaultModule = {};
var companyModule={};
var company={};
//重复提交标识
toaMachineNetworkfaultModule.subState = false;
//获取修改信息
toaMachineNetworkfaultModule.get = function (id) {
	$.ajax({
		type : "GET",
		url : getRootPath()+"/machine/toaMachineNetworkfault/get.action",
		data : {"id": id},
		dataType : "json",
		success : function(data) {
			if (data) {
				//清除验证信息
				hideErrorMessage();
				toaMachineNetworkfaultModule.clearForm();
				$("#toaMachineNetworkfaultForm").fill(data);
			}
		}
	});
};

toaMachineNetworkfaultModule.spCall = function(data, controlId) {
	//id
	var id = "";
	//姓名
	var name = "";
...
//添加修改公用方法
toaMachineNetworkfaultModule.saveOrModify = function (hide) {
	if(toaMachineNetworkfaultModule.subState)return;
		toaMachineNetworkfaultModule.subState = true;
	if ($("#toaMachineNetworkfaultForm").valid()) {
		var url = getRootPath()+"/machine/toaMachineNetworkfault/save.action";
		if ($("#id").val() != 0) {
			url = getRootPath()+"/machine/toaMachineNetworkfault/update.action";
		}
		var formData = $("#toaMachineNetworkfaultForm").serializeArray();
		jQuery.ajax({
			url : url,
			type : 'POST',
			cache: false,
			...
			success : function(data) {
				...
				if(data.success == "true"){
					msgBox.tip({
						...
						callback:function(){
							loadrightmenu("/machine/toaMachineNetworkfault/manage.action","","/machine/toaMachineNetworkfault/manage.action");
						}
					});
...
//清空表单数据
toaMachineNetworkfaultModule.clearForm = function(){
	$('#toaMachineNetworkfaultForm')[0].reset();
	hideErrorMessage();
};
//提取公司名称
