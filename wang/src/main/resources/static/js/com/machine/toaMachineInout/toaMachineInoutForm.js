//初始化方法
var toaMachineInoutModule = {};
var companyModule={};
var company={};
//重复提交标识
toaMachineInoutModule.subState = false;
//获取修改信息
toaMachineInoutModule.get = function (id) {
	$.ajax({
		type : "GET",
		url : getRootPath()+"/machine/toaMachineInout/get.action",
		data : {"id": id},
		dataType : "json",
		success : function(data) {
			if (data) {
				//清除验证信息
				hideErrorMessage();
				toaMachineInoutModule.clearForm();
				$("#toaMachineInoutForm").fill(data);
			}
		}
	});
};

toaMachineInoutModule.spCall = function(data, controlId) {
	//id
	var id = "";
	//姓名
	var name = "";
...
//添加修改公用方法
toaMachineInoutModule.saveOrModify = function (hide) {
	if(toaMachineInoutModule.subState)return;
		toaMachineInoutModule.subState = true;
	if ($("#toaMachineInoutForm").valid()) {
		var url = getRootPath()+"/machine/toaMachineInout/save.action";
		if ($("#id").val() != 0) {
			url = getRootPath()+"/machine/toaMachineInout/update.action";
		}
		var formData = $("#toaMachineInoutForm").serializeArray();
		jQuery.ajax({
			url : url,
			type : 'POST',
			cache: false,
			...
			success : function(data) {
				...
				if(data.success == "true"){
					msgBox.tip({
						...
						callback:function(){
							loadrightmenu("/machine/toaMachineInout/manage.action","","/machine/toaMachineInout/manage.action");
						}
					});
...
//清空表单数据
toaMachineInoutModule.clearForm = function(){
	$('#toaMachineInoutForm')[0].reset();
	hideErrorMessage();
};
//提取公司名称
