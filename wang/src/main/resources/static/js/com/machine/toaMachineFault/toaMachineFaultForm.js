//初始化方法
var toaMachineFaultModule = {};
var companyModule={};
var company={};
//重复提交标识
toaMachineFaultModule.subState = false;
//获取修改信息
toaMachineFaultModule.get = function (id) {
	$.ajax({
		type : "GET",
		url : getRootPath()+"/machine/toaMachineFault/get.action",
		data : {"id": id},
		dataType : "json",
		success : function(data) {
			if (data) {
				//清除验证信息
				hideErrorMessage();
				toaMachineFaultModule.clearForm();
				$("#toaMachineFaultForm").fill(data);
			}
		}
	});
};

toaMachineFaultModule.spCall = function(data, controlId) {
	//id
	var id = "";
	//姓名
	var name = "";
...
//添加修改公用方法
toaMachineFaultModule.saveOrModify = function (hide) {
	if(toaMachineFaultModule.subState)return;
		toaMachineFaultModule.subState = true;
	if ($("#toaMachineFaultForm").valid()) {
		var url = getRootPath()+"/machine/toaMachineFault/save.action";
		if ($("#id").val() != 0) {
			url = getRootPath()+"/machine/toaMachineFault/update.action";
		}
		var formData = $("#toaMachineFaultForm").serializeArray();
		jQuery.ajax({
			url : url,
			type : 'POST',
			cache: false,
			...
			success : function(data) {
				...
				if(data.success == "true"){
					msgBox.tip({
						...
						callback:function(){
							loadrightmenu("/machine/toaMachineFault/manage.action","","/machine/toaMachineFault/manage.action");
						}
					});
...
//清空表单数据
toaMachineFaultModule.clearForm = function(){
	$('#toaMachineFaultForm')[0].reset();
	hideErrorMessage();
};
...
$(document).ready(function($) {
	selectControl.init("controlTree", "department-departments", true, false,null);	//组织多选
});