//初始化方法
var toaMachineRestartRoomHandleModule = {};
//重复提交标识
toaMachineRestartRoomHandleModule.subState = false;
//获取修改信息
toaMachineRestartRoomHandleModule.get = function (id) {
	$.ajax({
		type : "GET",
		...
		success : function(data) {
			if (data) {
				//清除验证信息
				hideErrorMessage();
				toaMachineRestartRoomHandleModule.clearForm();
				$("#toaMachineRestartRoomHandleForm").fill(data);
...
toaMachineRestartRoomHandleModule.spCall = function(data, controlId) {
	//id
	var id = "";
	//姓名
	var name = "";
...
//添加留言
toaMachineRestartRoomHandleModule.restartExchange = function (hide) {
	if(toaMachineRestartRoomHandleModule.subState)return;
		toaMachineRestartRoomHandleModule.subState = true;
			...
			success : function(data) {
				...
				if(data.success == "true"){
					msgBox.tip({
						...
						callback:function(){
							loadrightmenu("/machine/toaMachineRestart/loadRoomHandle.action?id="+ids+"", "", "/machine/toaMachineRestart/loadRoomHandle.action?id="+ids+"");
						}
					});
...
//添加修改公用方法(机房)
toaMachineRestartRoomHandleModule.saveOrModifyRoomHandle = function (hide) {
	if(toaMachineRestartRoomHandleModule.subState)return;
		toaMachineRestartRoomHandleModule.subState = true;
	if ($("#toaMachineRestartRoomHandleForm").valid()) {
		if ($("#id").val() != 0) {
			url = getRootPath()+"/machine/toaMachineRestart/updateRoomHandle.action";
		}
		var formData = $("#toaMachineRestartRoomHandleForm").serializeArray();
		jQuery.ajax({
			url : url,
			type : 'POST',
			cache: false,
...
//清空表单数据
toaMachineRestartRoomHandleModule.clearForm = function(){
	$('#toaMachineRestartRoomHandleForm')[0].reset();
	hideErrorMessage();
};