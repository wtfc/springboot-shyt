infoMyPortal = {};
/**分页变量*/
infoMyPortal.pageRows = null;
/**我的发布分页对象*/
infoMyPortal.oTable = null;

/**我的发布信息显示列信息*/
infoMyPortal.oTableAoColumns = [
...
/**我的发布组装后台参数*/
infoMyPortal.oTableFnServerParams = function(aoData){
	//排序条件
	getTableParameters(infoMyPortal.oTable, aoData);
	//组装查询条件
...
//我的发布分页查询方法
infoMyPortal.initList = function () {
	if (infoMyPortal.oTable == null) {
		infoMyPortal.oTable = $('#infoMyTable').dataTable( {
			"iDisplayLength": infoMyPortal.pageRows,//每页显示多少条记录
			"sAjaxSource": getRootPath()+"/im/info/manageMyPortalList.action",//后台分页查询地址url
			"fnServerData": oTableRetrieveData,//查询数据回调函数
			"aoColumns": infoMyPortal.oTableAoColumns,//table显示列
			//传参
			"fnServerParams": function ( aoData ) {
				infoMyPortal.oTableFnServerParams(aoData);
			},
			aaSorting:[],//设置表格默认排序列
			//默认不排序列
		...
		//table不为null时重绘表格
		infoMyPortal.oTable.fnDraw();
	}
};

//清空编制查询区域
infoMyPortal.queryFormReset = function(){
	$('#infoMyForm')[0].reset();
	//发布人人员选择树清空
 ...
 * 删除信息
 */
infoMyPortal.deleteInfo = function (id) {
	var ids = String(id);
	if (ids.length == 0) {
		alertx("请选择要删除的记录");
		return;
	...
	msgBox.confirm({
		...
		success: function(){
			infoMyPortal.deleteCallBack(ids)
		}
	});
};
 ...
 * 删除信息回调方法
 */
infoMyPortal.deleteCallBack = function(ids) {
	$.ajax({
		type : "POST",
		...
		success : function(data) {
			if (data.success == "true") {
				msgBox.tip({
					...
					callback:infoMyPortal.initList()
				});
			}else {
				msgBox.info({
...
/**将信息置顶*/
infoMyPortal.toTop = function(id,modifyDate){
	jQuery.ajax({
...
/**阅读情况分页对象*/
infoMyPortal.readingStatusTable = null;
/**查看阅读情况*/
infoMyPortal.initReadList = function(id){
	//显示列信息
	infoMyPortal.readingStatusTableAoColumns = [
	...
	//组装后台参数
	infoMyPortal.readingStatusTableFnServerParams = function(aoData){
		//排序条件
		getTableParameters(infoMyPortal.readingStatusTable, aoData);
		//组装查询条件
		aoData.push({ "name": "infoId", "value": id});
	};
	
	//table对象为null时初始化
	if (infoMyPortal.readingStatusTable == null) {
		infoMyPortal.readingStatusTable = $('#readingStatusTable').dataTable( {
			sAjaxSource: getRootPath()+"/im/infoRead/manageList.action?time=" + new Date(),
			bPaginate: false,
			fnServerData: oTableRetrieveData,//查询数据回调函数
			aoColumns: infoMyPortal.readingStatusTableAoColumns,//table显示列
			fnServerParams: function ( aoData ) {//传参
				infoMyPortal.readingStatusTableFnServerParams(aoData);
			},
			aaSorting:[],//设置表格默认排序列
		...
		//table不为null时重绘表格
		infoMyPortal.readingStatusTable.fnDraw();
...
/**临时添加的详细页跳转方法*/
infoMyPortal.infoDetail = function(infoId){
	// 删除验证
	$.ajax({
		type : "POST",
		...
		success : function(data) {
			if (!data) {
				...
				//loadrightmenu("/im/info/infoDetail.action?id="+infoId+"&type=1&menuFlag=0&time=" + new Date(),"unknown");
 ...
 */
jQuery(function($){
	infoMyPortal.pageRows = TabNub > 0 ? TabNub : 10;
	//我的发布查询按钮绑定事件
	$("#infoMyQuery").click(infoMyPortal.initList);
	//我的发布重置按钮绑定事件
	$("#infoMyReset").click(infoMyPortal.queryFormReset);
	selectControl.init("controlTree1", "sendId-sendId", true, true);//人员单选
	selectControl.init("controlTree2", "sendDepid-sendDepid", true, false);//部门选择
	//日历控件重新初始化
	...
	$("#columnId").val($("#portalColumnId").val())
	//初始化列表
	infoMyPortal.initList();
});