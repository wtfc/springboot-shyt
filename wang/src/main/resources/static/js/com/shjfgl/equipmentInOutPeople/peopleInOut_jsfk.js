...
equipmentInOut.oTableAoColumns = [
	{mData: function(source) {
			...
			return "<input type=\"checkbox\" name=\"ids\" value="+ source.id +">";
				...
				return "<input type=\"checkbox\" name=\"ids\" hidden=\"hidden\">";
			}
		}
	},
	{ "mData": "companyName"},
...
//接单
equipmentInOut.deleteStatusEquipmentInOut = function (id) {
	var ids = String(id);
	if (id == 0) {
		var idsArr = [];
	...
	msgBox.confirm({
		content: $.i18n.prop("JC_SYS_129"),
		success: function(){
			equipmentInOut.deleteStatusTimeSet(ids);
		}
	});
};


equipmentInOut.deleteStatusTimeSet = function(ids) {
	$.ajax({
		type : "POST",
		url : getRootPath()+"/machine/equipmentInOut/deleteStatus.action",
		data : {"ids": ids},
		dataType : "json",
		success : function(data) {
			if (data.success == "true") {
				msgBox.tip({
					...
					callback:function(){
						loadrightmenu("/machine/equipmentInOut/managekfhdPeople.action","","/machine/equipmentInOut/managekfhdPeople.action");
						}
				});
			} else {
				msgBox.info({
