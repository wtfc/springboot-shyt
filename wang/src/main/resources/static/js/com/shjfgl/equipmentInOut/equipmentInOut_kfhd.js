...
//是否开始操作
equipmentInOut.operateEquipmentInOut = function (id) {
	var ids = String(id);
	if (id == 0) {
		var idsArr = [];
		$("[name='ids']:checked").each(function() {
...
equipmentInOut.deleteStatusTimeSet = function(ids) {
	$.ajax({
		type : "POST",
		url : getRootPath()+"/machine/equipmentInOut/operate.action",
		data : {"ids": ids},
		dataType : "json",
		success : function(data) {
			if (data.success == "true") {
				msgBox.tip({
					...
					content: data.successMessage,
					callback:function(){
						loadrightmenu("/machine/equipmentInOut/manageczz.action","","/machine/equipmentInOut/manageczz.action");
						}
				});
			} else {
				msgBox.info({
