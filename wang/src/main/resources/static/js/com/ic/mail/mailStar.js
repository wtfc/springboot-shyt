...
mail.options = {
	...
	listURL: "/ic/mail/manageStarList.action?t="+(new Date()).getTime(),
	mailbox:"1",
	index:0
};
...
mail.oTableAoColumns = [
		...
		{// 状态
			mData : function(source) {
				...
				if (source.receivers[0].readFlag == 1) {
					...
					status += "<a href=\"javascript:mail.setReadStatusById("
						+ source.id
						+ "," + source.receivers[0].id + ",1);\" id=\"lightread"+ source.receivers[0].id  + "\">"
		...
		{// 标题
			mData : function(source) {
				...
				if(source.receivers[0].readFlag == 1){
					return "<div class=\"email-theme red-dark\" id=\"m_title_"+ source.receivers[0].id  + "\" title=\"" + mailTitle + "\" style = \" cursor :pointer;\" onclick = mail.detail(" + source.id
...
//				return '<a class="dark" href="#email-list" role="button" data-placement="left" data-toggle="pomail" data-container="body" data-content="aaaaa"><i class="fa fa-users"  ></i></a>';
...
mail.setStar=function(id,folderId,mrids){
	$.ajax({
		...
		success : function(data) {
			if (data > 0) {
				$("#lightstar" + id).toggleClass("yellow-dark");
				if($("#m_title_" + id).attr("style")!=null)
					{
						$("#m_title_" + id).removeAttr("style");
					}
				else
					{
					$("#m_title_" + id).attr("style","color:red");
					}
				mail.mailList();
				//获得datatable分页当页第一条记录是中条数的第多少条
				start = $("#mailTable").dataTable().fnSettings()._iDisplayStart; 
...
//	loadrightmenu("/ic/mail/manageStarDetail.action?id="+id+"&index="+index+"&mrid="+mrid
//			+"&mailTitle="+mailTitle+"&senderMail="+sendMail+"&senderUserId="+senderUserIdNav+"&searchReceiveTimeBegin="+searchReceiveTimeBegin
//			+"&searchReceiveTimeEnd="+searchReceiveTimeEnd+"&receivers[0].readFlag="+receiversReadFlag+"&showNext="+showNext+"&showPre="+showPre+"&folderId="+folderId+"&returnURL="+url);
	jQuery.ajax({
		url : getRootPath()+"/ic/mail/manageStarDetail.action?id="+id+"&index="+index+"&mrid="+mrid+"&folderId="+folderId,
		type : 'POST',
		data:{
			'returnURL':url,
			'mailboxId':mailboxId,
...
mail.simpleSearch=function(){
	...
	if($('#simple-search-title').val()==null || $('#simple-search-title').val()=="")
	{
		...
		mail.options.listURL= "/ic/mail/manageStarList.action";
	}
	else{
		$('#mailEasyTitle').val($('#simple-search-title').val());
		mail.options.search=mail.constants.SEARCH_METHOD_SIMPLE;
		mail.options.listURL= "/ic/mail/manageSearchStar.action";
	}
	mail.oTable=null;
	mail.mailList();
};
