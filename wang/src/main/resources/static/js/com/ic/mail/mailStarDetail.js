					...
					success : function(data) {
						if (data > 0) {
							...
							if($("#index").val()==0
								...
								loadrightmenu("/ic/mail/manageStar.action");
...
mailDetail.navigate = function(forward) {
	$(".outgoing-mail").css("display","none");
	$.ajax({
		url : getRootPath() + "/ic/mail/navigateStar.action",
		type : 'GET',
		data : {
			...
			"index":$('#index').val(),
			"receivers[0].starMail":1,
			"receivers[0].readFlag":$('#receiversReadFlag').val(),
...
mailDetail.postPsw=function(){
	$.ajax({
		url : getRootPath() + "/ic/mail/manageStarDetail.action",
		type : 'GET',
		data : {"id":$('#id').val(),
			"mrid":$("#recId").val(),
			"index":$('#index').val(),
