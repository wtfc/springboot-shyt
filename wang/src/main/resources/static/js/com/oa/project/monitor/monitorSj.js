...
//设计与完成
monitor.updatemonitor = function (id) {
	var ids = String(id);
	if (id == 0) {
		var idsArr = [];
		$("[name='ids']:checked").each(function() {
	...
	msgBox.confirm({
		content: $.i18n.prop("JC_SYS_138"),
		success: function(){
			monitor.updateTimeSet(ids);
		}
	});
};


monitor.updateTimeSet = function(ids) {
	$.ajax({
		type : "POST",
		url : getRootPath()+"/project/monitor/updateStatus.action",
		data : {"ids": ids},
		dataType : "json",
		success : function(data) {
			if (data.success == "true") {
