//初始化方法
var toaNetworkPortModule = {};
var companyModule={};
var company={};
//重复提交标识
toaNetworkPortModule.subState = false;
//获取修改信息
toaNetworkPortModule.get = function (id) {
	$.ajax({
		type : "GET",
		url : getRootPath()+"/network/toaNetworkPort/get.action",
		data : {"id": id},
		dataType : "json",
		success : function(data) {
			if (data) {
				//清除验证信息
				hideErrorMessage();
				toaNetworkPortModule.clearForm();
				$("#toaNetworkPortForm").fill(data);
			}
		}
	});
};
//添加修改公用方法
toaNetworkPortModule.saveOrModify = function (hide) {
	if(toaNetworkPortModule.subState)return;
		toaNetworkPortModule.subState = true;
	if ($("#toaNetworkPortForm").valid()) {
		var url = getRootPath()+"/network/toaNetworkPort/save.action";
		if ($("#id").val() != 0) {
			url = getRootPath()+"/network/toaNetworkPort/update.action";
		}
		var formData = $("#toaNetworkPortForm").serializeArray();
		jQuery.ajax({
			url : url,
			type : 'POST',
			cache: false,
			...
			success : function(data) {
				...
				if(data.success == "true"){
					msgBox.tip({
						...
						callback:function(){
							loadrightmenu("/network/toaNetworkPort/manage.action","","/network/toaNetworkPort/manage.action");
						}
					});
...
//清空表单数据
toaNetworkPortModule.clearForm = function(){
	$('#toaNetworkPortForm')[0].reset();
	hideErrorMessage();
};
