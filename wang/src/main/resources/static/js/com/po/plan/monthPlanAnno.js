 ...
 * @author 刘锡来
 */
var monthPlanAnno = {};//月计划批注对象
monthPlanAnno.subState = false;//重复提交标识,false时,允许提交表单
 ...
 * 初始化数据加载
 */
monthPlanAnno.initMonthPlanAnno = function(piId) {
//	$("#planFormDiv").load(getRootPath()+"/po/plan/planAnnoDataLoad.action",null,null);//绘制表格
	var url = getRootPath() + "/po/plan/planWorkFlowInit.action?planType=1&time="+ new Date();
	$.ajax({
		type : "POST",
		url : url,
		dataType : "json",
		success : function(data) {
			...
			if(data.spanOn =="true"){
				for (var i=0;i< data.planList.length;i++) {
					if(i == 0){
						monthsStr += "<tr>";
						monthsStr += "<td id='"+ i +"' style='background-color:#ffc334' class='jiac' onclick=showOtherMonthPlan('"+data.planList[i].id+"',"+i+")><span>"
								  + data.showSpanList[i]
								  + "</span>月份</td>";
						monthsStr += "</tr>";
					}else{
						monthsStr += "<tr>";
						monthsStr += "<td id='"+ i +"' onclick=showOtherMonthPlan('"+data.planList[i].id+"',"+i+")><span>"
								  + data.showSpanList[i]
								  + "</span>月份</td>";
						monthsStr += "</tr>";
					}
			...
			$("#monthsTbody").append(monthsStr);
			
			var dataDetailsSum = data.detailPlan.sumDetails;
			var dataDetailsPlan = data.detailPlan.planDetails;
			monthPlanAnno.autoTrInit(dataDetailsSum.length,dataDetailsPlan.length);
			//填充工作总结详细数据
			...
			$("#otherSpanSumDiv").html(data.planList[0].sumSubmitYear+"年"+data.planList[0].sumSubmitMW+"月份工作总结");
			$("#otherSpanPlanDiv").html(data.planList[0].planSubmitYear+"年"+data.planList[0].planSubmitMW+"月份工作计划");
		...
		success : function(data) {
			if(data.isLeader == "true" ||sourceUser == targetUser){//当前登录人是计划申请人的上级或者当前登录人是计划申请人
				...
				$("#leaderIdea").click(function() {
					monthPlanAnno.saveLeaderIdeaForm(planId);
				});
				//绑定批注回复事件
				$('#comment').on('click', 'a[name="reply"]', function() {
					monthPlanAnno.commentReply(this);
				});
				//绑定批注发送按钮事件
				$('#comment').on('click', 'a[name="send"]', function() {
					monthPlanAnno.commentSend(this);
				});
				//绑定批注内容清除
...
//加载计划表单数据(没有数据)
monthPlanAnno.planFormLoadNoData = function() {
		...
		success : function(data) {
			...
			if (data.spanOn == "true") {
				for (var i = 0; i < data.planList.length; i++) {
					if(i == 0){
						monthsStr += "<tr>";
						monthsStr += "<td id='"+ i +"' style='background-color:#ffc334' onclick=showOtherMonthPlan('"+data.planList[i].id+"',"+i+")><span>"
								  + data.showSpanList[i]
								  + "</span>月份</td>";
						monthsStr += "</tr>";
					}else{
...
//				$("#planFormDiv").load(getRootPath()+"/po/plan/planAnnoDataLoad.action",null,null);//绘制表格
				...
				monthPlanAnno.planFormLoadNoData();
			}
		}
	});
}
 ...
 * 显示另外计划
 */
function showOtherMonthPlan(id,spanId){
	$("#dataLoad").fadeIn();
	$("#monthsSpanTable td").each(function(i,v) {
		v.style.cssText = "background-color:#fff";
	});
	$("#"+spanId).attr("style","background-color:#ffc334");
	$("#"+spanId).attr("name","monthSpanColor");
...
function planDetailLoad(id,spanId,flushFlag){
	...
	$.ajax({
		...
		success : function(data) {
			...
			var dataDetailsSum = data.detailPlan.sumDetails;
			var dataDetailsPlan = data.detailPlan.planDetails;
			monthPlanAnno.autoTrInit(dataDetailsSum.length,dataDetailsPlan.length,flushFlag);
...
//动态添加行初始化方法
monthPlanAnno.autoTrInit = function(sumLength, planLength,flushFlag) {
	//初始化工作总结动态增加行
	...
	indexFun(true, "preSum", sumLength, 1, autoTrSumStr,flushFlag);
...
//			+ "<td><span autoFlowForm='textinput' class='auto-style'><input type='text' class='fzrOnfocus' maxlength='0' onclick='selectControl.singlePerson(this.id,false,plan.singlePersonPlanCallBack)' id='tempId' name='tempName'></span></td>@"
	...
	indexFun(false, "prePlan", planLength, 1, autoTrPlanStr,flushFlag);
};

//领导批注初始化方法
monthPlanAnno.initAnno = function(dataid) {
	$.ajax({
		type:"GET",
...
//领导批注初始化方法
monthPlanAnno.initAnnoReadOnly = function(dataid) {
	$.ajax({
		type:"GET",
		url:getRootPath() + "/po/plan/queryLeaderIdea.action",
		data:{"businessId":dataid},
...
//领导批注保存方法
monthPlanAnno.saveLeaderIdeaForm = function(planId) {
...
//领导批注回复方法
monthPlanAnno.commentReply = function(e) {
...
//领导批注发送方法
monthPlanAnno.commentSend = function(e) {
				...
				success : function(data) {
					if (data.success == "true") {
						msgBox.tip({
							...
							callback : function() {
								$('#comment').empty();
								monthPlanAnno.initAnno(businessId);
							}
						});
					} else {
						msgBox.info({
...
//总结动态添加方法
function autoCreateColumnCode(temp,ValueTemp,flushFlag){//首次进入页面后，自动创建行(序号)
...
//计划动态添加方法
function autoCreateColumn(temp,ValueTemp,flushFlag){//首次进入页面后，自动创建行
...
function indexFun(isCodeTemp,tableIdTemp,autoNumValTemp,autoAddValTemp,autoContentTemp,flushFlag){
	...
	if(isCode){
		...
		autoCreateColumnCode(autoNumVal,6,flushFlag);
	}else{
		tablePrePlanId = tableIdTemp;
		autoContentPlan=autoContentTemp;
		autoCreateColumn(autoNumVal,6,flushFlag);
	}
}
...
//清空表单
monthPlanAnno.clearForm = function () {
	$('#zdsz')[0].reset();
	//清空人员选择树隐藏域
...
function toUpdatePlanAnno(){
	...
	$.ajax({
		type : "POST",
		url : url,
		data:{"planId":planId,"planType":1,"id":planId,"showYear":$("#showYearSpan").val(),
			  "targetUser":targetUser,"sourceUser":sourceUser},
		dataType : "json",
		success : function(data) {
			if(data.success == "true"){
				$("#showYearSpan").text(data.showYearSpan);//span年显示
 ...
 */
jQuery(function($) {
	monthPlanAnno.initMonthPlanAnno();
	$("#leftYear").click(function() {optMonthYear(-1);});
	$("#rightYear").click(function() {optMonthYear(1);});
	months_tree();
});