...
<script src="${sysPath}/js/com/ic/mail/mailStarDetail.js" type="text/javascript"></script>
		

<header class="con-header pull-in" id="navigationMenu">
	<h1>邮件内容</h1>
	<div class="crumbs">
		<a href="#">首页</a><i></i>互动交流<i></i>电子邮件<i></i>邮件内容
	</div>
</header>
		<section class="panel m-t-md">
				...
				<section class="email-btn form-btn">
					...
					<div class="inline fr">
						...
						<c:when test="${mail.index eq 0 || mail.index eq -10 || mail.showPre eq false}">
							<button id="btnPre" class="btn" type="button" onclick="mailDetail.navigate(-1)" disabled="disabled">上一封</button>
						</c:when>
						<c:otherwise>
						...
						<c:when test="${mail.index eq 999999 || mail.index eq -10 || mail.showNext eq false}">
							<button id="btnNext" class="btn m-r-none" type="button" onclick="mailDetail.navigate(1)" disabled="disabled">下一封</button>
						</c:when>
						<c:otherwise>
			...
			<header class="email-header table-wrap">
				...
				<div class="panel-heading clearfix dropup">
					...
					<div class="fr" id="outGoingDiv">
						<a href="#" class="btn outgoing-mail-down" role="button" data-toggle="modal">查看往来邮件</a>
					</div>
				...
				<div class="table-wrap corpus">
					...
					<input type="hidden" id="mailEasyTitleSearch"  value="${mailEasyTitle }"/>
					...
					<dl class="email-horizontal1 clearfix">
						...
						<dd class="controls">
						<p class="p-hide" id="senderMailEcho">
							<c:choose>
					...
					<dl class="email-horizontal1 clearfix">
						...
						<dd class="controls" >
							<p id="senderTime"><fmt:formatDate value="${mail.senderTime }" type="both"
				...
				<div class="table-wrap simple padder-v">
					...
					<c:if test="${mail.mailboxId ne 1 }"><b class="m-r-xs">发件人邮箱：</b> ${mail.senderMail }</c:if>
			...
			<c:choose>
				<c:when test="${empty mail.encryption or mail.encryption eq 0 }">
					...
					<div class="email-compile table-wrap hide" id="password">
						这是一封由 <strong>${mail.senderUserName }</strong> 发出的加密邮件，需要解密才能阅读。
			...
			<div class="form-table table-wrap">
				<input type="text" value="可快速回复：" class="fast-reply email-placeholder">
			</div>
			<!--end 邮件快速细回复-->
