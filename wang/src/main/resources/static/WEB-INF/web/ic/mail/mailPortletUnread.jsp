...
function loadmaildata(){
	jQuery.ajax({
        url: getRootPath()+"/ic/mail/getportalUnread.action?funViewType=${funViewType}&dataRows=${dataRows}&time="+new Date(),
        type: 'post',
        success: function(data, textStatus, xhr) {
        	if('${funViewType}' == 'pviewType_1'){
        		loadmailmixlist(data.mailList,data.dataRows,"mail1${portletId }${functionId}","dataLoad_mailUnread${portletId }${functionId}");
        	} else if('${funViewType}' == 'pviewType_9'){
        		loadmailmaxlist(data.mailList,data.dataRows,"mail9${portletId }${functionId}","dataLoad_mailUnread${portletId }${functionId}");
        	} else if('${funViewType}' == 'pviewType_7'){
        		loadmailshortcut(data.mailListSize,"mail7${portletId }${functionId}","dataLoad_mailUnread${portletId }${functionId}");
        	} else {
        		$("#dataLoad_mailUnread${portletId }${functionId}").fadeOut();
        	} 
        },error:function(e){
        	// alert("加载数据错误。");
...
function loadmailmixlist(infoList,dataRows,divname,loadingname){
   	var content = "";
  	var title = "";
  	if(infoList.length > 0){
  		for(var i=0;i<infoList.length;i++){
  	   		...
  	   		content = content + "<td style=\"display: none;\"></td>"+
  	   			"<td><a href=\"#\" title=\""+title+"\" onclick=\"loadrightmenu('/ic/mail/managePortletView.action?id="+infoList[i].id+"&showNext="+infoList[i].showNext+"&index="+infoList[i].index+"&mrid="+infoList[i].mrid+"','','/ic/mail/manageUnread.action');\">"+
  	   			title+"</a> </td>";
...
function loadmailmaxlist(infoList,dataRows,divname,loadingname){
   	var content = "";
   	var title = "";
   	if(infoList.length > 0){
   		for(var i=0;i<infoList.length;i++){
   	   		...
   	   		content = content +"<li><i class=\"fa fa-clipboard icon fl\"></i><div class=\"fl index-inform-con\">"+
   	   			"<a href=\"#\" title=\""+title+"\" onclick=\"loadrightmenu('/ic/mail/managePortletView.action?id="+infoList[i].id+"&mrid="+infoList[i].mrid+"','','/ic/mail/manageUnread.action');\">"+
...
<c:choose>
	...
	<c:when test="${funViewType == 'pviewType_7' }"><!-- 快捷方式 -->
		<i class="fa fa-inform icon fl"><b class="bg-green-dark rounded">&nbsp;</b></i>
         <div class="fl panel-con-wrap inform-con"> <strong id="mail7${portletId }${functionId}"></strong><a href="#" onclick="portalview.loadMore('${portletId }')"><span id="shortcutName"></span></a>  </div>
	</c:when>
...
<div class="loading" id="dataLoad_mailUnread${portletId }${functionId}"></div>