				...
				<section class="panel">
					<div class="table-wrap">
						...
						<form class="cmxform" id="mailForm">
							...
							<c:if test="${mail.mailboxId ne 1}">
								<c:choose>
									<c:when test="${not empty mail.attachs }">
											...
											<dd class="controls">
												...
												<c:forEach var="attach"
														...
														<c:choose>
															...
															<i class="fa fa-paper-clip"></i>
																<a
																	href="javascript:newmail.downloadExt('${attach.fileName }','${mailReplyId}')"
																	class="blue-dark m-r-xs">${attach.fileName }</a>&nbsp;
																	</c:otherwise>
														</c:choose>
													</c:forEach>
								...
								<table class="form-table">
									<tr style="height:50px;">
										...
										<td class="w100"><label class="checkbox inline innermail"><input
												...
												>加密</label></td>
										<td class="encryption-td" style="display: none;">
										<label
						...
						<!--业务关联表的属性值，新增附件时不需要填写或填写空字符串-->
			                ...
			                <input type="hidden" id="islogicDel" value="1">
			                 <input type="hidden" id="upload_div_name" value="fileUpload-edit">
               				 <input type="hidden" id="upload_close_callback" value="newmail.clearAttachForReply">
...
<script>
	$(document).ready(function(){
		...
		iePlaceholderPatch();
		var fastContent = window.opener.document.getElementById("fastContent").value;
		fastContent = fastContent.replace(/\n/g, "<br/>");
		$("#mailContent").val(fastContent+$("#mailContent").val());
	});
</script>
</html>