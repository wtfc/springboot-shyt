...
<style>
	*{
	...
	#shahe2033 tbody tr,#shahe2033 tbody tr td{
		/* border: 1px black solid; */
		width: auto;
		text-align: center;
	}

	#shahe2033 tbody tr td{
		height: 22px;
		line-height:22px;
		white-space:nowrap;
	}
...
<h1 class="panel-heading clearfix" style="text-align: center;font-size: 25px">沙河2033机房平面图</h1>
<div >
...
<table class="table table-striped tab_height first-td-tc" id="shahe2033">
<tbody>
	<%-- <tr>
		<td>
		...
		<c:forEach items="${dList}" var="m">
			<c:if test="${m.machinaNumber eq 'NE-7'}">  
	     		   style="background-color:${m.type};" style="background-color:${m.type};" onClick="add(2033,'${ m.machinaNumber}')" data-original-title="机柜编号：${ m.machinaNumber};是否开通：${m.isOpen} ; 机柜容量：${m.valume} ;设备数量：${m.num} ;"
			</c:if>
		</c:forEach>
		>NE-7</button>
	</tr> --%>
	...
	<tr>
		...
		<c:forEach items="${dList}" var="m">
			<c:if test="${m.machinaNumber eq 'A01'}">  
	     		   style="background-color:${m.type};" onClick="add(2033,'${ m.machinaNumber}')" data-original-title="机柜编号：${ m.machinaNumber};是否开通：${m.isOpen} ; 机柜容量：${m.valume} ;设备数量：${m.num} ;"
			</c:if>
		</c:forEach>
		>A01</button></td>
