...
<section class="scrollable padder jcGOA-section" id="scrollable"> 
	<section class="panel m-t-md" id="body">
			<h3 class="tc" style="margin:0;border:0;">森华易腾IDC设备进出审批单 </h3>
				...
				<table class="table table-td-striped">
					<tbody>
						...
						<tr>
							...
							<td style="width:10%;" class=" b-green-dark b-tc">下单时间</td>
							<td>
								<input type="text" readonly="readonly" style="width:100%;"
						...
						<tr>
							<td>设备信息</td>
							<td colspan="5"><textarea   id="deviceInfo"
								name="deviceInfo" ></textarea></td>
						</tr>
						<tr>
							<td>操作需求</td>
							<td colspan="5"><textarea   id="operate"
								name="operate" ></textarea></td>
						</tr>
						<tr>
							<td >备注说明</td>
              ...
              <form class="table-wrap  " id="equipmentInOutForm2">
               ...
               <table class="table table-td-striped">
						<tr>
							...
							<td style="width:20%"><select id="isInput" style="width:100%"name = "isInput">
								  <option selected value="" >请选择</option>
								  <option  value ="是">是</option>
								  <option value ="否">否</option>
								</select>
							</td>
							<td><span class="required">*</span>设备数量</td>
							<td><input  type="text"style="width:100%" id="countt" 
								name="countt" /></td>
							<td><span class="required">*</span>实施情况</td>
							<td>
								<select id="implemtation" style="width:100%" name = "implemtation">
								  <option selected value="" >请选择</option>
								  <option value ="全部加电加网">全部加电加网</option>
								  <option value="部分加电加网">部分加电加网</option>
								  <option value="未加电加网">未加电加网</option>
								  <option value="断电搬出">断电搬出</option>
								  <option value="已断电设备搬出">已断电设备搬出</option>
								  <option  value="无">无</option>
								</select>
							</td>
						</tr>
						<tr>
							<td><span class="required">*</span>是否客户操作</td>
							<td><select id="implemtationName" style="width:100%" name = "implemtationName">
								  <option selected value="" >请选择</option>
								  <option value ="是">是</option>
								  <option value="否">否</option>
								</select>
							</td>
							<td><span class="required">*</span>施工负责人</td>
							<td><input  type="text"style="width:100%" id="workLeader"value="${userName}"  name="workLeader" /></td>
							<td>操作工程师</td>
							<td>
							<textarea  id="caozgcs" name="caozgcs" 
							<c:if test="${status eq '5'}">
							onFocus="selectControl.singlePerson(this.id, true, 'equipmentInOutModule.spCall', eval($('#userJson').val()));"
							</c:if> 
							></textarea>
							<!-- <input  type="text" style="width:100%"  id="caozgcs" 
								name="caozgcs" /> --></td>
						</tr>
						<tr>
							<td >备注说明</td>
							<td colspan="5"><textarea   id="remark" name="remark" ></textarea>
							</td>
						</tr>
					</tbody>
					</table>
					...
					<c:if test="${status eq '5'}">  
						...
						<button class="btn dark" type="button" onclick=equipmentInOutModule.equipmentInOutForm2(true) id="equipmentInOutForm2">提 交</button>
						<%-- </shiro:hasPermission> --%>
					</c:if>
					</div>
	              	</form>
                  ...
                  <!-- tab-pane tab切换显示的模块 内部panel的类名替换为panel-in-box -->
               		<table class="table table-td-striped">
               			<tbody>
               			<tr>
							<td><span class="required">*</span>是否完成</td>
				...
				<c:if test="${status eq '2'}">  
					...
					<button class="btn dark" type="button" onclick=equipmentInOutModule.equipmentInOutForm4(true) id="equipmentInOutForm4">提 交</button>
					<%-- </shiro:hasPermission> --%>
				</c:if>
				</div>
              	</form>
							...
							<div class="table-wrap form-table" id="exchange">
								...
								<%-- <shiro:hasPermission name="user:add4"> --%>
									<button id="exchangeSubmit" class="btn"
										onclick=equipmentInOutModule.equipmentInOutForm3(true)
...
<script src="${sysPath}/js/com/shjfgl/equipmentInOut/module/equipmentInOutModule.js" type="text/javascript"></script>
<script src="${sysPath}/js/com/shjfgl/equipmentInOut/equipmentInOut.validate.js"type="text/javascript"></script>